...
def render_layout_plotly(results_dir, design_name):
    ...
    if best_def:
        try:
            ...
            # Parse COMPONENTS
            comp_sec = re.search(r'COMPONENTS\s+\d+\s*;(.*?)(?:END COMPONENTS|PINS)', content, re.DOTALL)
            if comp_sec:
    ...
    if not components:
        cell_types = [
            ("dfxtp_1", 2.76), ("xor2_1", 1.84), ("and2_0", 1.38), 
            ("nand2_1", 0.92), ("nor2_1", 0.92), ("inv_1", 0.92), 
            ("buf_1", 0.92), ("decap_8", 3.68), ("fill_1", 0.46)
        ]
        import random
    ...
    def get_cell_colors(cell_type):
        ...
        elif "and" in c or "or" in c or "xor" in c or "nand" in c or "nor" in c or "mux" in c or "maj" in c or "o21" in c or "a21" in c:
    ...
    for sx in stripe_x_vdd:
        ...
        annotations.append(dict(
            x=sx, y=die_h - 2, text="VDD", showarrow=False,
    ...
    for sx in stripe_x_vss:
        ...
        annotations.append(dict(
            x=sx, y=die_h - 2, text="VSS", showarrow=False,
    ...
    for cell in display_cells:
        ...
        if "width" in cell:
            ...
            h = 2.72
            if "dfxt" in c_name: w = 2.76
            elif "xor" in c_name: w = 1.84
            elif "and" in c_name or "or" in c_name or "nand" in c_name or "nor" in c_name: w = 1.38
            elif "inv" in c_name or "buf" in c_name: w = 0.92
            elif "decap" in c_name: w = 3.68
            else: w = 1.38
        ...
        scatter_x.append(cx)
        scatter_y.append(cy)
        scatter_text.append(f"Instance: {cell['instance']}<br>Cell: {cell_type}<br>X: {cx:.2f} um<br>Y: {cy:.2f} um")
    ...
    import random
    random.seed(42)
    wire_colors = ["rgba(0, 210, 211, 0.4)", "rgba(255, 107, 129, 0.4)", "rgba(255, 235, 59, 0.3)"]
    for i in range(25):
        if len(components) < 2: break
    ...
    for idx, pin_name in enumerate(inputs):
        ...
        fig.add_trace(go.Scatter(
            x=[0], y=[py],
            mode="markers",
            marker=dict(symbol="triangle-right", size=8, color="#00ff9d", line=dict(color="#00a8ff", width=0.5)),
            text=f"Pin: {pin_name} (INPUT)",
            hoverinfo="text",
            showlegend=False
        ))
        annotations.append(dict(
            x=2.5, y=py, text=pin_name, showarrow=False,
    ...
    for idx, pin_name in enumerate(outputs):
        ...
        fig.add_trace(go.Scatter(
            x=[die_w], y=[py],
            mode="markers",
            marker=dict(symbol="triangle-left", size=8, color="#ff9f43", line=dict(color="#ee5253", width=0.5)),
            text=f"Pin: {pin_name} (OUTPUT)",
            hoverinfo="text",
            showlegend=False
        ))
        annotations.append(dict(
            x=die_w - 2.5, y=py, text=pin_name, showarrow=False,
    ...
    fig.update_layout(
        title=dict(
            text=title_text,
            font=dict(size=13, color='#f8fafc', family='Arial')
        ),

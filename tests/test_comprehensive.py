...
class TestWaveformDisplay:
    ...
    def test_find_vcd_for_design_returns_none_when_missing(self):
        ...
        result = find_vcd_for_design(
            isolated, "totally_nonexistent_xyz_design"
        )
...
"""
    ...
    def test_verify_port_match_connection_strings(self):
        from universal_rtl_generator import (
            generate_matching_testbench, parse_module_ports
        )
        tb = generate_matching_testbench(self.ADDER_RTL, "adder_8bit")
        ports = parse_module_ports(self.ADDER_RTL)
...
class TestVerilogGeneratorUtils:
    ...
    def test_save_design_creates_files(self):
        ...
        assert "rtl_path" in result or "design_dir" in result or \
    ...
    def test_detect_sim_tool_returns_string(self):
        ...
        assert result in ["docker", "icarus", "cadence", "vivado", "none",
                          "Docker", "Icarus", "Cadence", "Vivado", "None"]
    ...
    def test_build_sim_result(self):
        ...
        assert isinstance(result, dict)
        assert "status" in result or "success" in result or \
               "pass" in str(result).lower()
...
"""
    ...
    def test_detect_module_type_counter(self):
        from universal_testbench import (
            parse_verilog_module, detect_module_type
        )
        info = parse_verilog_module(self.COUNTER_RTL)
...
class TestLayoutViewer:
    ...
    def test_get_gds_layer_info_missing_file(self):
        ...
        assert result.get("error") or result.get("layers", []) == [] or \
               "status" in result or len(result) == 0
...
class TestGeneratedTestbenches:
    ...
    def _count_tests(self, tb: str) -> int:
        ...
        task_patterns = [
            r'\bcheck_add\b', r'\bcheck_count\b', r'\bcheck_alu\b', r'\bcheck_mux\b',
            r'\bstart_txn\b', r'\bread_check\b', r'\bwrite_check\b', r'\bshift_in\b',
            r'\bcheck_transition\b', r'\bsend_byte\b', r'\bpass_test\b'
        ]
        ...
        for line in lines:
            line = line.strip()
            m = re.search(r'for\s*\(\s*(\w+)\s*=\s*(\d+)\s*;\s*\1\s*(<=|<)\s*([^;]+)\s*;', line)
            if m:
                start_val = int(m.group(2))
                op = m.group(3)
                limit_expr = m.group(4).strip()
                
                clean_expr = limit_expr.replace('{max_val}', str(max_val)).replace('max_val', str(max_val))
                try:
            ...
            if in_for_loop:
                has_check = any(re.search(pat, line) for pat in task_patterns) or '$display("PASS' in line or '$display("FAIL' in line or '$display("PASS Test' in line
                if has_check:
                ...
                has_check = any(re.search(pat, line) for pat in task_patterns) or ('$display("PASS' in line and "Test" in line) or ('$display("PASS Test' in line)
                if has_check:
                    total_tests += 1
...
class TestDesignDB:
    ...
    def test_creation(self):
        ...
        db = DesignDB(design_name="test_design", rtl_sources=["top.v"], netlist_path="top.v")
    ...
    def test_timing_roundtrip(self):
        from design_db import DesignDB, TimingData, TimingCorner, TimingPath, TimingPathCell
        ...
        db.timing = TimingData(period_ns=10.0,
            corners={"TT": TimingCorner(corner="TT", slack_ns=5.57, met=True,
                paths=[TimingPath(startpoint="a", endpoint="b", slack_ns=5.57, met=True,
                    cells=[TimingPathCell(delay=0.1, time=0.1, net="n1", cell="BUF")])])},
            fmax_mhz=225.73, hold_slack_ns=0.123)
        d = db.to_dict()
    ...
    def test_congestion_roundtrip(self):
        from design_db import DesignDB, CongestionData
        db = DesignDB(design_name="c", rtl_sources=["c.v"], netlist_path="c.v")
        cd = CongestionData(h_overflow_pct=0.05, v_overflow_pct=0.12, max_density_pct=52.3)
        cd.compute_score()
        db.congestion = cd
        d = db.to_dict()
        db2 = DesignDB.from_dict(d)
    ...
    def test_file_persistence(self):
        from design_db import DesignDB, save_design_db, load_design_db
        import tempfile
    ...
    def test_flow_population(self):
        from design_db import DesignDB, TimingData, TimingCorner, PowerData, CongestionData, DRCCheck, LVSCheck
        db = DesignDB(design_name="flow", rtl_sources=["f.v"], netlist_path="f.v")
        db.timing = TimingData(period_ns=10.0, corners={"TT": TimingCorner(corner="TT", slack_ns=5.57, met=True)}, fmax_mhz=225.73)
        db.power = PowerData(total_mw=0.0564, dynamic_mw=0.0563, leakage_uw=0.0058)
        cd = CongestionData(h_overflow_pct=0.0, v_overflow_pct=0.0, max_density_pct=35.0)
        cd.compute_score()
        db.congestion = cd
        ...
        assert db2.congestion is not None and db2.drc is not None and db2.lvs is not None

    def test_from_db_qor(self):
        from design_db import DesignDB, TimingData, TimingCorner, PowerData, CongestionData
    ...
    def test_summary_dict(self):
        from design_db import DesignDB, TimingData, TimingCorner
    ...
    def test_mcmm_roundtrip(self):
        ...
        mcmm_data = mcmm.MCMMTiming(
            corners={
                "TT": mcmm.TimingCorner(name="TT", worst_negative_slack=5.57, fmax_mhz=225.73, met=True),
                "SS": mcmm.TimingCorner(name="SS", worst_negative_slack=3.21, fmax_mhz=147.28, met=True),
                "FF": mcmm.TimingCorner(name="FF", worst_negative_slack=-0.05, fmax_mhz=99.50, violations=1, met=False),
            },
            period_ns=10.0,
        )
    ...
    def test_spef_roundtrip(self):
        ...
        spef_data = spef.SPEFResult(
            ...
            nets=[spef.ParasiticNet(net_name="n1", wire_length_um=1200.0, resistance_ohm=96.0, capacitance_pf=0.24, delay_impact_ps=23.04)],
        )
    ...
    def test_drc_engine_roundtrip(self):
        ...
        drc_eng = drc.DRCEngineResult(
            total_violations=2,
            violations=[drc.DRCViolation(rule_name="width", layer="metal1", x=1.0, y=2.0)],
            by_rule={"width": 2},
            by_layer={"metal1": 2},
            by_severity={"error": 2},

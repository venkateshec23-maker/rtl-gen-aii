...
runs_dir = Path(os.getenv("OPENLANE_RUNS", "C:/tools/OpenLane/runs"))
designs = [
    "adder_8bit",
    "simple_alu",
    "counter",
...
for d in designs:
    candidates = sorted(
        runs_dir.glob(f"{d}_*"), key=lambda p: p.stat().st_mtime, reverse=True
    )
    if not candidates:
    ...
    print(
        f"{d:15s} | {rd.name:45s} | GDS:{gds_ok:3s} {gds_kb:>5}KB | routed:{routed_kb:>5}KB | cts:{cts_kb:>5}KB"
    )

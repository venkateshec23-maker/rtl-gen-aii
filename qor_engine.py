...
def _build_power_tcl(
    ...
    sdc_section = f"read_sdc {sdc_linux}" if sdc_linux else f"create_clock -name clk -period 10 [get_ports clk]"
    return f"""
...
# Load routed DEF (includes netlist + placement, no need for read_verilog/link_design)
read_def {def_path}

# Clock constraints for switching activity
{sdc_section}
...
def run_power_analysis(
    ...
    netlist_candidates = (
        list(run_dir_windows.rglob(f"{design_name}_sky130.v")) +
        list(run_dir_windows.rglob(f"{design_name}_synth.v")) +
        list(work_dir_windows.rglob(f"{design_name}_sky130.v"))
    )
    netlist_windows = netlist_candidates[0] if netlist_candidates else None
    if netlist_windows is None:
        log.warning("Power analysis skipped: synthesized netlist not found")
        return empty

    # ── Locate SDC constraints ───────────────────────────────────────
    sdc_candidates = (
        list(run_dir_windows.rglob(f"{design_name}.sdc")) +
        list(run_dir_windows.rglob("*.sdc"))
    )
    sdc_windows = sdc_candidates[0] if sdc_candidates else None
    ...
    sdc_linux        = to_linux(sdc_windows) if sdc_windows else ""

    # -- Generate and run TCL ------------------------------------------
    tcl_content = _build_power_tcl(design_name, run_dir_linux, netlist_linux, routed_def_linux, sdc_linux)
...
def build_qor_report(
    ...
    # ── Hold slack (parse hold analysis report — no Docker call) ───────
    hold_path = run_dir_windows / "hold_analysis.txt"
    if hold_path.exists() and hold_path.stat().st_size >= _MIN_REPORT_BYTES:
        ...
        if sta_ff_path:
            ...
            qor.warnings.append("Hold analysis report not found — hold slack not measured")

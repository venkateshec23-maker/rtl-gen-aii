...
# Generated by qor_engine.py — do not edit manually
...
read_def {run_dir_linux}/routed.def
...
def run_power_analysis(
    ...
    # ── Locate routed DEF ────────────────────────────────────────────
    routed_def = run_dir_windows / "routed.def"
    if not routed_def.exists() or routed_def.stat().st_size < 1000:
        log.warning("Power analysis skipped: routed.def missing or too small")
        return empty
    ...
    tcl_content = _build_power_tcl(design_name, run_dir_linux, netlist_linux)
...
def _build_congestion_tcl(design_name: str, run_dir_linux: str) -> str:
    return f"""
...
def run_congestion_analysis(
    ...
    run_dir_linux = to_linux(run_dir_windows)
    tcl_content   = _build_congestion_tcl(design_name, run_dir_linux)

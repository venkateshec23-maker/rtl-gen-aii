...
# Base paths
WORK_DIR = Path(os.getenv("OPENLANE_WORK",
                          r"C:\tools\OpenLane"))
RUNS_DIR  = WORK_DIR / "runs"
INDEX     = RUNS_DIR / "index.json"
REPORTS_DIR = Path(r"C:\Users\venka\Documents\rtl-gen-aii\reports")


def find_results_dir(
    design_name: str,
    hint_dir: Optional[str] = None
) -> Optional[Path]:
    """
    ...
    def has_pipeline_output(d: Path) -> bool:
        ...
        try:
            return (d / "sta_final.txt").exists() or \
                   bool(list(d.glob("*.gds")))
        except:
            return False
    ...
    if INDEX.exists():
        try:
            ...
            if isinstance(runs, list):
                ...
                matching = [
                    r for r in runs
                    if r.get("design_name") == design_name
                    and r.get("results_dir")
                ]
                if matching:
                    latest = matching[-1]
    ...
    if RUNS_DIR.exists():
        candidates = sorted(
            [d for d in RUNS_DIR.iterdir()
             if d.is_dir() and design_name in d.name],
...
def parse_timing(results_dir: Path, filename: str) -> Dict:
    ...
    if not f.exists():
        return {"status": "NOT_RUN", "slack_ns": 0,
                "wns_ns": 0, "met": False}
...
def parse_lvs(results_dir: Path) -> Dict:
    """Parse LVS report."""
    for fname in ["lvs_report_final.txt",
                  "lvs_report.txt"]:
        f = results_dir / fname
        if f.exists():
            content = f.read_text(errors="ignore")
            ...
            # Count devices
            device_m = re.search(
                r'Number of devices:\s+(\d+)', content
            )
            net_m = re.search(
                r'Number of nets:\s+(\d+)', content
            )
            devices = int(device_m.group(1)) if device_m else 0
            nets    = int(net_m.group(1)) if net_m else 0

            return {
                "status":  "MATCHED" if matched
                           else "MISMATCH" if mismatch
                           else "PARTIAL",
                "devices": devices,
                "nets":    nets,
                "file":    str(f)
...
def parse_gds(results_dir: Path) -> Dict:
    ...
    return {"name": "NOT_FOUND", "size_kb": 0,
            "real": False, "size_b": 0}
...
def parse_drc(results_dir: Path) -> Dict:
    ...
    for fname in ["drc_report.txt", "drc.log",
                  "magic_drc.txt"]:
        f = results_dir / fname
        if f.exists():
            content = f.read_text(errors="ignore")
    ...
    if xml_files:
        ...
        return {
            "violations": count,
            "status": _get_drc_status(count, True)
        }
...
def parse_synthesis(results_dir: Path,
                    design_name: str) -> Dict:
    """Parse synthesis metrics."""
    for fname in [f"{design_name}_synth.log",
                  "synthesis.log",
                  "yosys.log"]:
        f = results_dir / fname
        if f.exists():
...
def collect_all_data(
    ...
    data["timing_tt"] = parse_timing(
        results_dir, "sta_final.txt"
    )
    data["timing_ss"] = parse_timing(
        results_dir, "sta_ss.txt"
    )
    data["timing_ff"] = parse_timing(
        results_dir, "sta_ff.txt"
    )

    # All corners check
    corners = [data["timing_tt"],
               data["timing_ss"],
               data["timing_ff"]]
    run_corners = [c for c in corners
                   if c["status"] != "NOT_RUN"]
    data["all_corners_met"] = all(
        c["met"] for c in run_corners
    ) if run_corners else False
    ...
    data["verdict"] = (
        ...
        and data["lvs"]["status"] in
           ("MATCHED", "MATCHED_WITH_WARNINGS")
        and data["gds"]["real"]
...
def generate_pdf(data: Dict,
    ...
    def status_color(s):
        if s in ("MET","MATCHED","PROVEN",
                 "PASS","REAL","DONE","TAPE-OUT READY"):
            return GREEN
        if s in ("NOT_RUN","INCONCLUSIVE",
                 "SKIPPED","UNKNOWN"):
            return ORANGE
        return RED
    ...
    story.append(Paragraph(
        "<b>RTL-to-GDSII Sign-Off Report</b>",
        styles["Title"]
    ))
    story.append(Paragraph(
        "Generated by RTL-Gen AI - Open Source EDA",
        styles["Normal"]
    ))
    ...
    info = [
        ...
        ["Results Path",  data["results_dir"][-60:]
                          + ("..." if len(data["results_dir"]) > 60
                             else "")],
    ]
    ...
    checks = [
        ...
        ["DRC",
         drc["status"],
         f"{drc['violations']} violations "
         f"(Magic + KLayout full rule deck)"],

        ["LVS",
         lvs["status"],
         f"{lvs['devices']} devices matched, "
         f"{lvs['nets']} nets"],

        ["Timing (TT)",
         tt["status"],
         f"Slack: {tt['slack_ns']:.2f} ns "
         f"@ 100 MHz (25C, 1.8V)"],

        ["Timing (SS)",
         ss["status"],
         f"Slack: {ss['slack_ns']:.2f} ns "
         f"@ 100C, 1.6V (worst setup)"],

        ["Timing (FF)",
         ff["status"],
         f"Slack: {ff['slack_ns']:.2f} ns "
         f"@ -40C, 1.95V (worst hold)"],

        ["Formal Equiv",
         frml["status"],
         "RTL == synthesized netlist (Yosys SAT)"],
    ...
    phys_rows = [
        ...
        ["GDS Export", "COMPLETE" if gds["real"] else "PENDING", 
         f"Output: {gds['name']} ({gds['size_kb']} KB)"],
        ["DRC Clean", "YES" if drc["violations"] == 0 else "NO",
         f"{drc['violations']} violations found"],
        ["LVS Clean", "YES" if lvs["status"] == "MATCHED" else "PENDING",
         lvs["status"]],
    ]
    ...
    # TIMING SUMMARY
    # ================================================
    if any(c["status"] != "NOT_RUN"
           for c in [tt, ss, ff]):
        story.append(Paragraph(
            "<b>Multi-Corner Timing Analysis</b>",
            styles["Heading2"]
        ))
        story.append(Spacer(1, 4))
    ...
    log_rows = [
        ...
        ["8. LVS Check", "Netgen", "PASS" if lvs["status"] == "MATCHED" else "SKIP", "~3s"],
    ...
    # ================================================
    story.append(Paragraph(
        "<b>Tape-Out Requirements Checklist</b>",
        styles["Heading2"]
    ))
    story.append(Spacer(1, 4))
    ...
    reqs = [
        ...
        ["Timing Closure", "WNS >= 0 at all corners", "ACHIEVED" if tt["met"] else "FAILED"],
        ["DRC Clean", "Zero DRC violations", "ACHIEVED" if drc["violations"] == 0 else "FAILED"],
        ["LVS Match", "Schematic matches layout", "ACHIEVED" if lvs["status"] == "MATCHED" else "PENDING"],
        ["GDS Valid", "File > 50KB with layers", "ACHIEVED" if gds["real"] else "FAILED"],
        ["Formal Equiv", "RTL == Netlist", "ACHIEVED" if frml["status"] == "PROVEN" else "PENDING"],
        ["Multi-corner", "TT/SS/FF corners MET", "ACHIEVED" if data["all_corners_met"] else "PENDING"],
    ...
    v_text = data["verdict"]
    v_color = "#00aa44" if "READY" in v_text \
              else "#cc0000"

    story.append(Paragraph(
        f'<b>Overall Verdict: '
        f'<font color="{v_color}">{v_text}</font></b>',
        styles["Heading1"]
    ))
    ...
    story.append(HRFlowable(
        width="100%", thickness=0.5, color=GRAY
    ))
    story.append(Spacer(1, 6))
...
def generate_signoff_report(
    ...
    if rd is None:
        ...
        rd = Path(results_dir) if results_dir \
    ...
    if output_path is None:
        ...
        output_path = str(
            REPORTS_DIR / f"{design_name}_{ts}_signoff.pdf"
        )
    ...
    try:
        ...
        with open(txt, "w") as f:
            ...
            f.write(f"  TT:     {data['timing_tt']['status']}"
                    f" {data['timing_tt']['slack_ns']}ns\n")
            f.write(f"  SS:     {data['timing_ss']['status']}"
                    f" {data['timing_ss']['slack_ns']}ns\n")
            f.write(f"  FF:     {data['timing_ff']['status']}"
...
if __name__ == "__main__":
    ...
    rd   = sys.argv[2] if len(sys.argv) > 2 \
           else None

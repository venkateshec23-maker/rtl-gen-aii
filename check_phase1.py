...
RUNS = Path(os.getenv("OPENLANE_RUNS", "C:/tools/OpenLane/runs"))
LATEST = {
...
def check(design, dirname):
    ...
    is_fb = j.get("status") == "FALLBACK" or (
        "fallback" in str(gds_file.name).lower() if gds_file else False
    )
...
print(
    f"{'Design':15s} | size | real | no_fb| drc  | lvs  | setup| fmax | power| hold | route| to   | PASS?  Details"
)
print("=" * 140)
passed = 0
